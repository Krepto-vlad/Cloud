...
def get_users():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT UserID, name, email FROM UladzislauBarsukou_registration.users"
    )
...
def get_user(user_id: int):
    ...
    cursor.execute(
        ...
        " FROM UladzislauBarsukou_registration.users WHERE UserID = ?",
        user_id,
    )
    row = cursor.fetchone()
    conn.close()
...
def get_registrations():
    ...
    cursor.execute(
        ...
        " FROM UladzislauBarsukou_registration.registrations"
    )
    rows = cursor.fetchall()
    conn.close()
...
def create_registration(data: RegistrationCreate):
    ...
    logger.info(
        "POST /registrations userID=%d tornamentId=%d", data.userID, data.tornamentId
    )
    conn = get_connection()
    try:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO UladzislauBarsukou_registration.registrations"
            " (userID, tornamentId, status)"
        ...
        try:
            ...
            conn.rollback()
            logger.error(
                "Failed to publish event for registerID=%d: %s", register_id, exc
            )
            raise HTTPException(
                status_code=503,
                detail="Unable to publish registration event. Please retry.",
            ) from exc
